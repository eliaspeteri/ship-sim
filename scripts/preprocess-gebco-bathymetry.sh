...
# Requires: gdalwarp, gdal_translate, gdal_merge.py, gdal2tiles.py, gdalbuildvrt, unzip
...
# 3. Tile the resampled GeoTIFF for LOD/performance
...
if [ ! -d "$TILE_DIR" ] || [ -z "$(ls -A "$TILE_DIR" 2>/dev/null)" ]; then
  echo "Tiling $TEMP_TIF into $TILE_DIR at zoom levels $ZOOM_LEVELS ..."
  mkdir -p "$TILE_DIR"
  gdal2tiles.py -z $ZOOM_LEVELS -w none "$TEMP_TIF" "$TILE_DIR"
else
